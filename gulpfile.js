...
gulp.task("sass", function() {
	var stream = gulp
		...
		.pipe(touch())
		.pipe(rtlCSS()) // Convert compiled styles to RTL.
		.pipe(
			rename({
				suffix: "-rtl"
			})
		) // Append "-rtl" to the converted RTL files.
		.pipe(sourcemaps.write("./")) // Output RTL source maps.
		.pipe(gulp.dest(paths.css)); // Output RTL stylesheets.
	return stream;
});
...
gulp.task("cssnano", function() {
	return gulp
		.src(`${paths.css}/theme?(-rtl).css`) // Add regex to look for "theme.css" or "theme-rtl.css".
		.pipe(sourcemaps.init({ loadMaps: true }))
		.pipe(
			plumber({
				errorHandler: function(err) {
